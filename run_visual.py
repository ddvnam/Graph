...
SURGE_OVERLAY     = (255, 200,  50, 60)   # RGBA — dùng để tô nền khi surge
...
class Renderer:
    ...
    # Draw grid
    # ------------------------------------------------------------------
    def draw_grid(self, grid: List[List[int]], surge_active: bool):
        ...
        # Surge overlay — tô nhẹ toàn lưới
        if surge_active:
            surf = pygame.Surface(
                (self.cell * self.cols, self.cell * self.rows), pygame.SRCALPHA
            )
            surf.fill((255, 200, 50, 45))
            self.screen.blit(surf, (self.grid_offset_x, self.grid_offset_y))
    ...
    def draw_panel(self, obs: dict, result_dict: Optional[dict] = None):
        ...
        # Surge indicator
        cfg      = self.stepping.env.cfg
        in_surge = any(ts <= t <= te for ts, te in cfg.get("surge_windows", []))
        if in_surge:
            surge_surf = self.font_h.render("⚡ SURGE ACTIVE", True, (255, 200, 50))
            self.screen.blit(surge_surf, (px + 8, h - 50))
    ...
    def run(self):
        ...
        while running:
            ...
            cfg      = self.stepping.env.cfg
            in_surge = any(ts <= obs["t"] <= te for ts, te in cfg.get("surge_windows", []))

            self.draw_grid(grid, in_surge)
...
def main():
    ...
    parser.add_argument("--config-index", type=int, default=0,     help="Chỉ số config trong file (mặc định: 0)")
